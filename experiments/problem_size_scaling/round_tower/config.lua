...
experiment_name = "Round towers"

-- Specify a comment that is displayed in the HTML report. It must be formatted as HTML code 
-- (so HTML special characters must be escaped or they will be interpreted as HTML code).
comments = nil

-- Name of the executable to analyze
executable     = "../../../build/Delta2"
...
run_command    = "<executable>  -t 0.1 -s 13 --print_priority 1 -n 1 --local_ts --scenario_size 64 --scenario_size2 12"
...
batch_script    = "runtime-maqao.sbatch"
...
-- This filter can use a different loop count for VPROF analysis and DECAN analysis
--filter = {
--   type = "number",
--   nvprof = 10,
--   ndecan = 10,
...
-- Specify a minimal threshold on VPROF metrics (cycles_per_iteration_mean * iteration_mean)
-- a loop must have to be analyzed using DECAN.
-- decan_threshold = 500
