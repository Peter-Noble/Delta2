...
State updateState(State& current, double t, const Vector3d& force, const Vector3d& force_offset, const Vector3d& torque, const Vector3d& torque_offset, Particle* p) {
    ...
    updated.setTranslation(current.getTranslation() + t * updated.getVelocity() + t * t * force_offset / p->getMass());
...
void SequentialImpulsesSTime::solve(collision::Cluster& cluster, std::vector<collision::Contact<double>>& hits) {
    ...
    for (int it = 0; it < 10; it++) {
        ...
        for (int c = 0; c < hits.size(); c++) {
            ...
            auto F = [=](double dist) { return (outer - inner) / ((dist - inner)*(dist - inner)) - (outer - inner) / ((outer - inner)*(outer - inner)); };
            ...
            if (s > 0) {
                contacts[c].force_move_offset = (ds - d0) * Meff / std::pow(s * dt, 2);
            }
            else {
    ...
    for (int p = 0; p < cluster.particles.size(); p++) {
        ...
        if (!cluster.particles[id].is_static) {
            ...
            State FState = updateState(SState, (1 - min_s[id])*cluster.step_size*cluster.step_size, force, force_offset, torque, torque_offset, &cluster.particles[id]);
    ...
    for (int it = 0; it < 10; it++) {
        for (int c = 0; c < hits.size(); c++) {
            ...
            auto Heaviside = [](double f) { return f >= 0.0 ? 1.0 : 0.0; };
            auto eq = [=](double dc, double d1) { return d1 + dc - ds - dt*vs*(1 - s) - std::pow(dt, 2)*std::pow(1 - s, 2)*(F_total + 0.5*std::max(0.0, (-inner + outer)/std::pow(-inner + std::max(0.0, std::min(ds, outer)), 2) - 1/(-inner + outer)) + 0.5*std::max(0.0, (-inner + outer)/std::pow(-inner + std::max(0.0, std::min(outer, d1 + dc)), 2) - 1/(-inner + outer)))/Meff; };
            auto grad = [=](double dc) { return 1 + 1.0*std::pow(dt, 2)*std::pow(1 - s, 2)*(-inner + outer)*Heaviside((-inner + outer)/std::pow(-inner + std::max(0.0, std::min(outer, d1 + dc)), 2) - 1/(-inner + outer))*Heaviside(-d1 - dc + outer)*Heaviside(std::min(outer, d1 + dc))/(Meff*std::pow(-inner + std::max(0.0, std::min(outer, d1 + dc)), 3)); };
