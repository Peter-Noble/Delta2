...
//     updated.setTranslation(current.getTranslation() + t * current.getVelocity() + 0.5 * force / p->getMass() * t * t + 0.5 * force_offset / p->getMass() * t * t);
...
//     int pre_S_iterations = 20;
//     int pre_S_contact_iterations = 100;
//     int pre_S_friction_iterations = 10;
//     int s_calc_iterations = 100;
//     int new_S_iterations = 100;
//     int contact_point_s_calc_iterations = 100;
//     int post_S_iterations = 20;
//     int post_S_contact_iterations = 10;
//     int post_S_friction_iterations = 10;
...
//     double separation_damp = 0.0;  // The proportion of energy used for a contact when it's separating

//     double s_damp = 0.1;
//     int primary = 0;
//     double s = 0.0;
//     model::FrictionSolver pre_S_friction_solver(cluster.particles, hits);
//     if (hits.size()) {

//         for (int it = 0; it < pre_S_iterations; it++) {
//             for (int contact_it = 0; contact_it < pre_S_contact_iterations; contact_it++) {
//                 // printf("Iteration: %i\n", it);
//                 double s_last = s;
...
//                     {
//                         Eigen::Vector3d force = forces[a_id] + pre_S_friction_solver.friction_forces[a_id];
//                         Eigen::Vector3d torque = torques[a_id] + pre_S_friction_solver.friction_torques[a_id];
...
//                     {
//                         Eigen::Vector3d force = forces[b_id] + pre_S_friction_solver.friction_torques[b_id];
//                         Eigen::Vector3d torque = torques[b_id] + pre_S_friction_solver.friction_torques[b_id];
...
//                         {
//                             Eigen::Vector3d force = forces[p] + pre_S_friction_solver.friction_forces[a_id];
//                             Eigen::Vector3d torque = torques[p] + pre_S_friction_solver.friction_torques[a_id];
...
//                     for (int p = 0; p < cluster.particles.size(); p++) {
//                         forces_last[p] = forces[p] + pre_S_friction_solver.friction_forces[p];
//                         torques_last[p] = torques[p] + pre_S_friction_solver.friction_torques[p];
//                         forces[p] = external_force(cluster.particles[p]);
//                         torques[p] = { 0.0, 0.0, 0.0 };
//                         force_offsets[p] = { 0.0, 0.0, 0.0 };
//                         torque_offsets[p] = { 0.0, 0.0, 0.0 };
//                     }
...
//                         if (it == pre_S_iterations - 1 && new_s_it == new_S_iterations - 1 && contact_it == pre_S_contact_iterations - 1) {
//                             printf("Contact pre S: %i, d0: %f, ds: %f, v0: %f, vs: %f\n", c, d0, ds, v0, vs);
//                         }
...
//                             double undamped = contacts[c].force_last;
//                             contacts[c].force_last = forceDamp(contacts[c].force_last, v0, vs, separation_damp);
//                             damp_diff = undamped - contacts[c].force_last;
//                         }

//                         Eigen::Vector3d force = -n * contacts[c].force_last;
//                         Eigen::Vector3d force_offset = -n * damp_diff;
...
//                         Eigen::Vector3d T_A_offset = model::calcTorque(force_offset, ps_A, SStates[a_id].getTranslation());
//                         Eigen::Vector3d T_B_offset = model::calcTorque(Eigen::Vector3d(-force_offset), ps_B, SStates[b_id].getTranslation());
...
//                         force_offsets[a_id] += force_offset;
//                         force_offsets[b_id] -= force_offset;
//                         torque_offsets[a_id] += T_A_offset;
//                         torque_offsets[b_id] += T_B_offset;
//                     }

//                     // Average the force across time steps
...
//                         if (!cluster.particles[p].is_static)
//                         {
//                             Eigen::Vector3d force = forces[p] + pre_S_friction_solver.friction_forces[p];
//                             Eigen::Vector3d torque = torques[p] + pre_S_friction_solver.friction_torques[p];
//                             Eigen::Vector3d force_offset = force_offsets[p];
//                             Eigen::Vector3d torque_offset = torque_offsets[p];
//                             double t = s * cluster.step_size;
...
//             // pre_S_friction_solver.solve(pre_S_friction_iterations, forces, torques, SStates, s * cluster.step_size);
//         }
//     }
...
//             Eigen::Vector3d force = forces[id] + pre_S_friction_solver.friction_forces[id];
//             Eigen::Vector3d torque = torques[id] + pre_S_friction_solver.friction_torques[id];
//             double t = s * cluster.step_size;
...
//     // Solve post S
//     model::FrictionSolver post_S_friction_solver(cluster.particles, hits);
//     if (hits.size() > 0 && s < 1.0) {
//         for (int it = 0; it < post_S_iterations; it++) {
//             for (int contact_it = 0; contact_it < post_S_contact_iterations; contact_it++) {
//                 // for (int c = 0; c < hits.size(); c++) {
//                 //     uint32_t a_id = cluster.particles.getLocalID(hits[c].p_a->id);
//                 //     uint32_t b_id = cluster.particles.getLocalID(hits[c].p_b->id);
...
//                     if (it == post_S_iterations - 1 && contact_it == post_S_contact_iterations - 1) {
...
//                 // Average force over last time step
...
//                     uint32_t id = cluster.particles.getLocalID(cluster.particles[p].id);
//                     if (!cluster.particles[id].is_static) {
//                         Eigen::Vector3d force = forces[id] + post_S_friction_solver.friction_forces[id];
//                         Eigen::Vector3d torque = torques[id] + post_S_friction_solver.friction_torques[id];
...
//                 }
//             }
//             // post_S_friction_solver.solve(post_S_friction_iterations, forces, torques, FStates, s * cluster.step_size);
//         }
