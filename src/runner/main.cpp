...
int main_old(int argc, char *argv[])
{
    ...
    __itt_string_handle* setup_task = __itt_string_handle_create("Setup");
    ...
    Delta2::timestepping::ExplicitAdaptiveClustersScheme ts(&particles, [](const Delta2::Particle& p) {
    ...
    int step = 0;
    while (cont) {
        __itt_string_handle* step_task = __itt_string_handle_create("Step");
        __itt_task_begin(domain, __itt_null, __itt_null, step_task);

        printf("Step: %i\n", step);
        std::vector<std::pair<Eigen::Vector3d, Eigen::Vector3d>> view_draws;
        ts.step(opt.time_step_size, view_draws);
        for (Delta2::Particle& p : particles) {
            if (!p.is_static) {
                // p.current_state.setVelocity(p.current_state.getVelocity() * 0.99);
                // p.current_state.setAngular(p.current_state.getAngularMomentum() * 0.99);
            }
        }
        if (opt.gui) {
            view.recordFrame(view_draws, particles[0].current_state.getTime());
