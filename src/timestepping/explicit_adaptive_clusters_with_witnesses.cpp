#include "explicit_adaptive_clusters_with_witnesses.h"
...
timestepping::ExplicitAdaptiveClustersWithWitnessesScheme::ExplicitAdaptiveClustersWithWitnessesScheme(std::vector<Particle> *ps, std::function<Eigen::Vector3d(const Particle &)> external_force, common::Options& o) : timestepping::TimestepScheme(ps, external_force, o)
{
...
void timestepping::ExplicitAdaptiveClustersWithWitnessesScheme::step(double time, std::vector<std::pair<Eigen::Vector3d, Eigen::Vector3d>> &view_draws)
{
    ...
    collision::fineWitnessCollisionClustersWithTimeStepSelection(*_particles, cluster_particles, cluster_interactions, cluster_step_size, cluster_sleeping, min_current_times, _cache);
